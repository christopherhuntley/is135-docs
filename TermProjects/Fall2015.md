...
3. Set up the repository:
  * **One team member** should click the `Set Up in Desktop` button to clone the repository. Open the repository folder in Atom and add the following:
    ...
    * A file enamed `Readme.md` which display the team name, the members, and a list of links to other files. The list of links should be empty for now.
...
4. Prepare for shared editing.  
The editing workflow goes like this:
  1. In GitHub Desktop hit the `Sync` button to get the latest version of the file.
  2. Edit the file in Atom.
  3. Use GitHub Desktop to `Commit` and `Sync` your work back to GitHub.
...
5. Draft a [Persona Analysis](https://en.wikipedia.org/wiki/Persona_(user_experience) document that describes each segment of the audience for the Showcase event. Each Persona profile should be one page and include all essential data needed to answer why and how the person would choose to attend the event.  
